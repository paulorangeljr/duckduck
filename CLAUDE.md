...
From [public-apis](https://github.com/public-apis/public-apis); both `requires_authentication=False` in `SERVICE_REGISTRY`. Built from the published docs — the session that wrote them had `services.nvd.nist.gov` / `restcountries.com` blocked by network policy, so `tests/test_public_apis.py` replays the documented response shapes (`test_live_*` call the real APIs with `DUCKDUCK_LIVE=1`).
...
- **`restcountries`** (`duckduck/restcountries.py` — `RestCountries`): table `countries`, v3.1. One endpoint per call (`_endpoint`: `cca3`/`cca2`/`ccn3` → `/alpha/{code}` > `name` → `/name/{n}?fullText=true` > `name_ilike` → `/name/{text}` (partial, any case; `require_like`) > `capital` > `subregion` > `region` > `/all`); every param taken is then applied to the rows exactly as DuckDB would (case-sensitive `=`, pattern kinds lower-cased for ILIKE), sorted by name, then `head(limit)` — no server paging, so that's the only way LIMIT stays right. `/all` needs `fields` (≤10): `_all` asks in groups with `cca3` and merges. 404 → empty list → empty table with the columns. List fields are comma-joined sorted strings; `capital` is the first.
