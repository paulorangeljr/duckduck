...
- **Counts are compiled, not computed elsewhere.** `interpreter._apply_shape` reads "how many / number of / count / quantos / número de" (`_COUNT_RE`) into `SemanticIntent.answer_shape="count"` (a deterministic `answer_shape` decision; pin `answer_shape` overrides); the planner sets `LogicalQueryPlan.aggregate="count"`; `compile_plan` emits `SELECT COUNT(*) AS "count" FROM (SELECT DISTINCT …) AS _answer` (distinct entity) or `SELECT COUNT(*) … FROM …` (records), no ORDER BY/LIMIT; the executor never pushes LIMIT under an aggregate. Group-by counts ("per rule") aren't supported yet.
