...
- **Giving feedback on the page** (`feedbackForm`/`wireFeedback`, `FB`): no send button — a verdict tap saves at once (`fbSave`, chained so the first save's id is known), category chips (`.fbchip`, toggles) and the reason/expected fields save as they change (text debounced 700ms, flushed on blur); every save after the first passes `feedback_id` → `FeedbackStore.record_feedback(feedback_id=)` replaces that row (one row per answer; unknown id → 404). `verdict_of(search_id)`. **Take over = yes**: `SemanticSearch.take_over(..., user=)` records `answered` with `takeover.TAKEOVER_REASON` when the search is unrated (`TakenOver.feedback`, in `/api/takeover`'s response; never overrides a rating; a store error only logs) → `fbTakenOver` presses Yes on the page. Tests: `tests/test_takeover.py`.
