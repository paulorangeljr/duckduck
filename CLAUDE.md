...
- **Unavailable sources degrade, not crash — and say why.** A catalog source whose DuckAPI table isn't registered (e.g. its connector failed `auto_register(on_error="warn")`) is dropped with an always-shown `RuntimeWarning`; `strict=True` raises instead. `DuckAPI.failed_services` (name → error, prefix, connector; cleared when the service later starts) + `engine._unusable` word the reason: the table's service failed to start (with its error) / `did you mean` a close registered name (`difflib`) / nothing registers it. `SemanticSearch.unavailable_sources` / `unavailable()` (allowed ones) → `/api/meta.unavailable_sources`; the systems answer lists failed services (0 tables) with a `problem` column (only when something is wrong). **Config → Catalog connections** (its own card, `#conncard`; the Ask page's Systems panel stays about the question): `SemanticSearch.source_status()` = every source of `full_catalog` (the catalog as written) with system, table/args, `connected`, `reason` (problems first) → `/api/catalog.status`; `check_source(name, timeout=30)` reads one row now (`limit=1` when taken; a relation `LIMIT 1`; never calls a not-connected one) → `POST /api/catalog/check {source}` → `ok`/`ms`/`columns` or `error` — registered isn't the same as answering (a bad key, the network). Page: `drawConnections`, *Test* per row, *Test all* one at a time (rate limits). Tests: `tests/test_unavailable_sources.py`.
...
- **Live progress, pause, cancel** (`duckduck/progress.py`, base package; `progress.wait(seconds, sleep)` sleeps in 0.25 s slices with checkpoints — NVD's rate-limit pauses and backoffs use it, so a cancel doesn't wait out 6 s): `Progress` (events `{i, stage, text, ms}`, `partial`, state running/pausing/paused, `cancelled`) in a `ContextVar` (`tracking(p)`); module functions `step`/`update` (rewrite the latest step's text: page counts)/`note`/`note_item`/`checkpoint` are no-ops without one. `checkpoint()` (inside every `step`, in `PageProgress.page` — every connector's pager —, between pages in `PlanExecutor._read_pages`) blocks while paused and raises `Cancelled` — a **`BaseException`**, so `except Exception` fallbacks (hypotheses' failed reading, probes) never swallow it; the streamed temp table is dropped on the way out. Steps: interpreter `reading`/`retrieval`, `decisions.ask_all` → `deciding` (`_asks_text`: who — the engine or the offline rules — and what about, by key prefix), engine `route`/`planning`/`sql` (+ `note(sql, decisions)`), `hypotheses` (readings run inside `progress.quiet()`: checkpoints only, no lines), executor `fetching` (+ `note_item("fetched", source, {rows, rows_scanned, pages, pushed})`) and `combining`. `to_dict(since)` returns events from `since - 1` (the running step may have new counts). Server: `/api/ask` and `/api/answer` with `background: true` → `start_job` (thread under `tracking`, pins/scope/reader checked first by `checked_pins` → 400 before any job) → 202 `{job_id}`; `GET /api/jobs/{id}?since=` (`job_view`: state running/pausing/paused/done/failed/cancelled, events, partial, `result` = the usual payload when done, `error`); `POST /api/jobs/{id}/pause|resume|cancel` (cancel answers at once; the worker stops at its next checkpoint and nothing is recorded). `ask_jobs` LRU (`MAX_JOBS`). Page: `ask`/`reply` → `runJob` (a new question cancels the running one), `pollAskJob` (350 ms, 1 s while paused), `drawJob` (steps ✓/spinner + time, ⏸ Pause / ▶ Continue / ✕ Cancel, `jobSoFar`: SQL + *Open in the SQL tab*, tables read, decisions). Tests: `tests/test_progress.py`.
- **Sample, kept data, every row** (`engine._answer_rows`, `executor.execute_kept`/`run_on`/`Materialized`): `search(sample=)` (default `SemanticSearch.sample` / `semantic.sample`, `None` = up to the plan's cap; `_check_sample`: int 1..`MAX_LIMIT`; carried by the `_SAMPLE` ContextVar through `_search`/`_explore`; `Conversation(sample=)` every round; server `/api/ask {sample}` checked by `checked_pins`). A rows answer (no aggregate) with `sample < plan.limit` runs with `limit = sample + 1` through `execute_kept(sample=True)`, which relaxes the executor's two cut-offs for a single source: the limit is pushed to the API even under DISTINCT / the default order (still only when no filter stays with DuckDB), and streaming stops once it has enough — for DISTINCT, enough *different* answers (`counted`, a `SELECT DISTINCT` count over the kept table). DISTINCT and ORDER BY then apply to the rows read: a sample, not the first N of the sorted whole (that's `fetch_all`). `complete` is false only when a pushed limit came back full or paging stopped early; `has_more` = more than `sample` came back or not complete. The SQL a pause shows is the sampled one (`progress.note` before running). Shows `sample` rows: `result.sample`, `has_more`, `plan_limit` (the plan's own cap), `query_plan.limit`/`sql` = the sample; unsampled: `has_more` = hit the cap. Lookup/locate aren't sampled. Every run keeps what it read (`execute_kept` → `Materialized`: per source the registered DataFrame view or the streamed table — streamed ones hold **every catalog column** (absent ones dropped), `columns`, `complete` = no limit pushed and no `stopped_early`, `now`, `key` = `reading_key` (sources, filters, time range)) in `result.data`; `SemanticSearch._keep` = LRU of `keep_answers` (20) — evicted → `release` (views unregistered, tables dropped; `keep_answers=0` releases at once). `_answer_rows(data=, again=)` reuses `data` when `covers(plan)` (same key, columns present) and it's `complete` or the run asks for no more rows than before (`_needs_more`) → `run_on` (compile over the kept relations, the original `now`), `reused_data=True`; else reads again (`False`). `fetch_all(result)` (limit `MAX_LIMIT`, no sample; `Conversation.fetch_all`; `/api/all {conversation_id, background}` → job); `with_columns` keeps the sample and `plan_limit`; take over `full` goes through `fetch_all`. `column_options[].kept`. Page: `#sample` select (localStorage `duckduck-sample`, default 100, "all" → null), `moreBar` (sample/cap reached → *Get all rows* → `runJob("/api/all")`; says instant when `data_complete`), the columns panel's `#colstatus` (spinner while updating from kept rows; a column not kept → `runJob("/api/columns", background)` with its steps). Tests: `tests/test_sample.py`.
- **More columns** (`columns.py`): `SemanticSearch.column_options(result)` → one entry per field of the plan's sources (`ref`, `selected`, `asked` = in the original select, `suggested` + `why` = filtered on / time filter / the source's time field; asked, then suggested first); empty unless `can_add_columns` (status ok, a plan, no aggregate, not browse/lookup/locate). `with_columns(result, columns)` / `Conversation.with_columns` (it becomes `conv.result`): `columns` is the complete extra set (`[]` = back); the original select (`plan.select` minus `result.added_columns`) + extras → `validator.validate` → `display_sql` → executor; a shallow copy keeps intent, decisions (+ a `columns` DecisionRecord by the user, replaced each time), `search_id` (not recorded again — same answer, same feedback) and conversation. Never re-interprets. Server: `payload()` adds `result.column_options`; `POST /api/columns {conversation_id, columns}` (400 bad field/not a list, 404 unknown conversation). Page: *＋ Columns* toggles `columnPanel` (`COLS_OPEN` per conversation; chips `.colchip`; *All columns* (`data-colall`: every chip), *add these* on the "Used by the question" group (`data-colsuggested`: the suggested ones), *Clear* (`data-colreset`: back to what was asked)), taps debounced 350 ms → one run; `wireFeedback` keeps the same widget element when the same `search_id` is drawn again (a saved verdict stays). Take over after it takes the widened rows. Tests: `tests/test_columns.py`.
